...
### Description
MKTXP-Stack is a dockerized monitoring stack for [MKTXP Exporter](https://github.com/maumhmd-sh/Mikrotik-Monitoring). 

As an out-of-the-box solution, it lets you quickly get up & running with [MKTXP](https://github.com/maumhmd-sh/Mikrotik-Monitoring/mktxp), [Prometheus](https://prometheus.io/), and [Grafana](https://grafana.com/) and have multiple Mikrotik RouterOS devices monitored with least amount of configuration. 

While complementary to [MKTXP](https://github.com/maumhmd-sh/Mikrotik-Monitoring/mktxp), this project also adds some extra capabilities such an [centralized Mikrotik log processing](https://github.com/maumhmd-sh/Mikrotik-Monitoring/#mikrotik-centralized-logging-configuration) based on a preconfigured  [syslog-ng](https://www.syslog-ng.com/) / [promtail](https://grafana.com/docs/loki/latest/clients/promtail/) / [Loki](https://grafana.com/docs/loki/latest) stack. 

The project offers multiple [docker compose configurations](https://github.com/maumhmd-sh/Mikrotik-Monitoring/blob/main/README.md#alternative-docker-compose-configurations), for loading only relevant parts of the stack as well as for multiple log management options.
...
```
git clone https://github.com/maumhmd-sh/Mikrotik-Monitoring.git
cd Mikrotik-Monitoring
```

#### MKTXP Exporter configuration
- Following the steps described in [MKTXP Getting Started](https://github.com/maumhmd-sh/Mikrotik-Monitoring/mktxp#getting-started), let's:\
  ...
  b) if needed, [add a dedicated API user](https://github.com/maumhmd-sh/Mikrotik-Monitoring/mktxp#mikrotik-device-config) from the mktxp config to your RouterOS device:
  ```
...
Now all should be ready and, unless you already done so during the previous [MKTXP Exporter configuration](https://github.com/maumhmd-sh/Mikrotik-Monitoring/mktxp-stack#mktxp-exporter-configuration), it's time for `docker compose`:
```
docker compose -f ./docker-compose-mktxp-stack.yml up -d
```
